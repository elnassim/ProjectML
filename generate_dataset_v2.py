...
# --- Paramètres Généraux ---
N_RECORDS = 40000
FILENAME_V2 = "dataset_revenu_marocains_v2.csv"
...
def generate_revenu_annuel_v2(df):
    n = len(df)
    revenus = np.zeros(n)
...
# --- Fonctions pour Imperfections (simplifiées) ---
def add_valeurs_manquantes_v2(df, colonnes_pour_nan, p_nan=0.001):  # Réduction de p_nan
...
def add_valeurs_aberrantes_revenu_v2(revenus_col, p_aberrant=0.0005):  # Réduction de p_aberrant
    n_aberr = int(p_aberrant * len(revenus_col))
    for _ in range(n_aberr):
        idx = np.random.randint(0, len(revenus_col))
        revenus_col[idx] = np.random.choice([100, 300000])  # Valeurs aberrantes limitées
    return revenus_col
...
def generate_dataset_v2():
    ...
    data['Revenu_Annuel'] = generate_revenu_annuel_v2(data.copy()) # Utilise la nouvelle fonction
    ...
    data = add_valeurs_manquantes_v2(data, cols_with_nan, p_nan=0.001)
    
    # Aberrations sur le revenu (après génération et NaN)
    # S'assurer de ne pas mettre d'aberration sur un revenu déjà NaN
    revenu_not_na_mask = data['Revenu_Annuel'].notna()
    data.loc[revenu_not_na_mask, 'Revenu_Annuel'] = add_valeurs_aberrantes_revenu_v2(data.loc[revenu_not_na_mask, 'Revenu_Annuel'].copy())
    ...
    # Pour les inactifs, si le revenu est généré trop haut par erreur, le ramener à une valeur plus basse
    # ou le laisser tel quel si la génération spécifique aux inactifs a déjà eu lieu.
    # La fonction generate_revenu_annuel_v2 gère déjà un revenu spécifique pour les inactifs.
    # On peut juste s'assurer qu'il ne dépasse pas un certain seuil s'il n'est pas NaN.
    ...
    ordered_columns = [
        ...
        'Propriete_immobiliere', 'Vehicule_motorise', 'Terrain_agricole',
        'Revenu_secondaire', 'Revenu_Annuel'
    ]
...
if __name__ == "__main__":
    print(f"Génération de {N_RECORDS} enregistrements avec le script V2...")
    dataset_v2 = generate_dataset_v2()
    
    print("\n--- Statistiques Descriptives du Revenu Annuel (V2) ---")
    print(dataset_v2['Revenu_Annuel'].describe())
    
    print("\nMoyennes par Milieu (V2):")
    print(dataset_v2.groupby('Milieu')['Revenu_Annuel'].mean())

    print("\nMoyennes par CSP (V2):")
    print(dataset_v2.groupby('CSP')['Revenu_Annuel'].mean().sort_values(ascending=False))

    print("\nMoyennes par Niveau d'éducation (V2):")
    print(dataset_v2.groupby('Niveau_education')['Revenu_Annuel'].mean().sort_values(ascending=False))

    dataset_v2.to_csv(FILENAME_V2, index=False, encoding='utf-8')
    print(f"\nDataset '{FILENAME_V2}' généré avec succès ({len(dataset_v2)} enregistrements).")
